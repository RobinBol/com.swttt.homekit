...
function configServer(homey) {
  this.config = new HAS.Config(homey.hostname, '82:E6:B6:63:BC:2C', HAS.categories.bridge, '../userdata/homey.json', 8091, '200-20-200');
  ...
  var identify = HAS.predefined.Identify(1, undefined, function(value, callback) {
      console.log(homey.hostname + ' Identify', value);
...
function addLight(device) {
  ...
  var lightIdentify = HAS.predefined.Identify(1, undefined, function(value, callback) {
    api.setDeviceState(device.id, 'onoff', true);
    api.setDeviceState(device.id, 'onoff', false);
    callback(HAS.statusCodes.OK);
  });
  // Set light details
  ...
  if (device.capabilities.dim) {
    var brightness = HAS.predefined.Brightness(9, device.state.dim * 100 || 0.0, (value, callback) => {
      debounce(api.setDeviceState(device.id, 'dim', value / 100), 500);
      callback(HAS.statusCodes.OK);
    });
