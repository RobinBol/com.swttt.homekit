...
async function getDeviceState(deviceid, capability) {
  var args = {
    headers: {
      "Content-Type": "application/json",
      "Authorization": "Bearer " + bearertoken
  ...
  const setDevice = await client.putPromise(apiUrl + "manager/devices/device/" + deviceid + "/state/", args);
  return setDevice.data.result[capability];
}
